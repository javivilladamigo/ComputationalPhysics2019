...
r = 5; k = 1.; P = 1e-5 # condiciones iniciales y definicion del paso de integracion
deltat = 0.1; t = 0
...
plt.clf()
error = 0; listapuntoserror = [] # error es una variable que nos dirá cuantos puntos hemos calculado con un paso de integración fuera del intervalo (a veces nos veremos obligados a hacerlo para que este paso no diverja, sobre todo en los tramos rectos de la logística pues el programa verá que las P's se diferencian poco y querrá hacer el paso cada vez más grande
for i in range(1000):
	k1 = deltat*f(P,t)
	...
	while (np.abs(Pnew-P)>precminima or np.abs(Pnew-P)<precmaxima): # bucle que se ejecuta mientras el paso no esté en el intervalo
		...
		# aquí abajo realmente la segunda condición del 'and' no hace falta pero para asegurarme
