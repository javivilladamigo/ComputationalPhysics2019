# Javier Mariño
## UPSTREAM TRANSPORTE ##
import matplotlib.pyplot as plt
import numpy as np

dimt = 300 # número de iteraciones temporales
deltat = 0.01; deltax = 0.5; 
C = 0.96; alpha = 0.5 # estabilidad: C+2s<=1
u = C*deltax/deltat 
s = alpha*deltat/deltax**2
N = 200 # parámetros naturales de la discretización

T = np.zeros(N+1); T[5:30]=np.ones(25)*1; #T[N]=10 # vector inicial, con las c.i. que queramos
Tnew = T.copy()

plt.close('all')
plt.plot(np.arange(N+1)*deltax,T,'-r'); plt.pause(0.00001) # primera representacion

for n in range(dimt):

	for i in range(1,N):
		Tnew[i]=(1-C)*T[i]+C*T[i-1]+(alpha*(T[i-1]-2*T[i]+T[i+1])/deltax**2)*deltat # calculamos el siguiente vector punto por punto
	Tnew = 0.5*(T+Tnew) # para evitar el rizado  
	T = Tnew.copy()
	#T[0]=0 #aplicamos cc
	T[N]=T[N-1]
	
	if n%10==0:
		plt.plot(np.arange(N+1)*deltax,T) #un plot un poco más discretizado para liberar memoria de python
		plt.pause(0.00001)
		plt.show()

plt.xlabel('$x$')
plt.ylabel('$T$')
plt.title('upstream transporte')